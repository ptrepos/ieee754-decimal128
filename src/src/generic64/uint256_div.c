...
// ’·œ–@‚É‚æ‚éÀ‘•
int mg_uint256_div_long_division(
	mg_uint256 *op1, 
	int op1_digits, 
...
int mg_uint256_div_maclaurin_impl(
	mg_uint256 *op1,
	const mg_uint256 *op2,
	int op2_bits,
...
MG_PRIVATE int mg_uint256_div_long_division(
				mg_uint256 *op1, 
				int op1_digits, 
				const mg_uint256 *op2, 
				int op2_digits, 
	...
	/*out*/mg_uint256 *quotient)
{
	...
	while (mg_uint256_is_zero(&xn) == 0) {
		...
		assert(!overflow);
		overflow = mg_uint256_add(&tmp2, &xn, /*out*/&series);		// È—ª‚µ‚½1‚Ì•ªA1 * xn‚ğ‰ÁZ
		assert(!overflow);
	}
	...
	overflow = mg_uint256_add(&tmp2, &tmp3, /*out*/&q);		// È—ª‚µ‚½1‚Ì•ª‚ğ‰ÁZ
	assert(!overflow);
