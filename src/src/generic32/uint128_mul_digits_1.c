...
MG_PRIVATE void mg_uint128_mul_digits_1(const mg_uint128 *op1, int op1_words, const mg_uint128 *op2, int op2_words, /*out*/mg_uint128 *low, /*out*/mg_uint128 *high)
{
	uint8_t carry, carry2;
	uint32_t lo, hi;
	uint32_t buf[MG_UINT128_SIZE * 2 + 1] = { 0 };
	...
	memcpy(low->word, buf, sizeof(buf[0]) * MG_UINT128_SIZE);
	memcpy(high->word, buf + 4, sizeof(buf[0]) * MG_UINT128_SIZE);
}
